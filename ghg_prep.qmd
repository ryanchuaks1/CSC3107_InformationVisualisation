...
world <-
  ...
  mutate(
    # Only show the top 10 based on total emissions
    country_label = if_else(min_rank(desc(Total_Emissions)) <= 10, name, ""),
    label_colour = if_else(min_rank(desc(Total_Emissions)) <= 10, 
                           "white", 
                           "transparent",
                           missing = "transparent"
                          ),
    pct_country_label = if_else(min_rank(desc(pct_total_emissions)) <= 10, paste0(round(pct_total_emissions, 2), "%"), ""),
    pct_label_color = if_else(is.na(pct_total_emissions), "transparent", "white")
  )
world
...
```{r}
#| label: source1-worldmap-plot

gg_ghg_total_emissions <- gg_atm("Top GHG Emitters") +
...
gg_ghg_pct <-
  gg_atm("Percentage of Total GHG Emissions") +
...
gg_ghg_total_emissions / gg_ghg_pct
```

...
# Select only the "Energy", Electricity/Heat, Agriculture, group the rest under "Other Sectors"
treemap_data <-
  treemap_data |>
  group_by(Country) |>
  mutate(Sector = if_else(Sector %in% c("Energy",
                                        "Electricity/Heat",
                                        "Agriculture",
                                        "Transportation",
...
# Select top 3 (via rank) sectors for each country and group the rest under "Other Sectors"
# treemap_data <-
#   treemap_data |>
#   group_by(Country) |>
#   mutate(Sector = if_else(rank(-Emissions) <= 3, Sector, "Other Sector")) |>
#   ungroup()

# Combine the emissions of the "Other Sector" for each country
treemap_data <-
  treemap_data |>
  group_by(Country, Sector) |>
  summarize(Emissions = sum(Emissions), .groups = "drop") |>
...
# Change the sector label of "Other Sectors" for "Others" Country to "Other Country"
treemap_data <-
  treemap_data |>
  group_by(Country) |>
...
# Change "European Union(27)" to EU27, change "Others" to "184 Remaining Countries"
treemap_data <-
  treemap_data |>
  mutate(Country = if_else(Country == "European Union (27)", "EU27", Country)) |>
  mutate(Country = if_else(Country == "Others", "184 Remaining Countries", Country))

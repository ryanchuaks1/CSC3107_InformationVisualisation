...
world <-
  ...
  mutate(
    country_label = if_else(min_rank(desc(Total_Emissions)) <= 10, paste0(rank(desc(Total_Emissions)), ". ", name), ""),
    label_colour = if_else(min_rank(desc(Total_Emissions)) > 2, "black", "white"),
    pct_country_label = if_else(min_rank(desc(pct_total_emissions)) <= 10, paste0(round(pct_total_emissions, 2), "%"), ""),
    pct_label_color = if_else(!is.na(pct_total_emissions) & min_rank(desc(pct_total_emissions)) > 2, "black", "white")
  )
world
...
gg_ghg_total_emissions <- gg_ghg("Top GHG Emitters") +
